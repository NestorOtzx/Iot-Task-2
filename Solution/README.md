...
Al finalizar las actividades, tu arquitectura debe verse exactamente como el siguiente diagrama, incorporando una base de datos documental (RDSDB), procesamiento sin servidor y una API en ECS:
...
graph TD
    ...
    subgraph "Nube (AWS Learner Lab)"
        ...
        L -->|Lógica Histórico| RDSDB[(RDSDB\nHistórico)]
    end
...
1. **Añadir RDSDB:**
   Modificar la infraestructura para aprovisionar una base de datos RDSDB. Configurar los accesos correspondientes.
...
3. **Lógica de Mantenimiento Histórico en Lambda:**
   Programar la Lambda para que lea el JSON de S3 y lo inserte en **RDSDB** para mantener el histórico completo de los eventos de cada sensor.
...
4. **Desarrollar una API REST Unificada:**
   ...
   - `GET /sensor/{id}/history`: Consulta el histórico completo en **RDSDB**.
